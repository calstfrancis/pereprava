...
#
# Everything runs inside main() so the whole script is read from stdin before
# anything executes — required when piped into `bash`, since a subprocess that
# touches stdin directly (any of the curl calls below) can otherwise race with
# bash's own read of the piped script and corrupt or truncate it.
set -euo pipefail
